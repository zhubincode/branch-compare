...
function getAllCommits(branchName, targetBranch, author, timeRange) {
  try {
    ...
    let baseCommand = `git log --pretty=format:"--COMMIT-START--%n%H%n%an%n%s%n%b%n--COMMIT-END--%n%ai"`;
    ...
    try {
      ...
      // 使用特殊标记分隔每个 commit
      const commitBlocks = output.split("--COMMIT-START--\n").filter(Boolean);

      const commits = commitBlocks
        .map((block) => {
          if (!block.includes("--COMMIT-END--")) {
            return null;
          }
          ...
          // 找到 --COMMIT-END-- 的位置
          const endIndex = lines.findIndex((line) => line === "--COMMIT-END--");
          if (endIndex === -1) {
            return null;
          }
          ...
          // body 是从第4行到 --COMMIT-END-- 之前的所有行
          const bodyLines = lines.slice(3, endIndex);
          const body = bodyLines.join("\n").trim();

          // date 是 --COMMIT-END-- 之后的第一行
          const date = lines[endIndex + 1];

          if (!hash || !authorName || !message || !date) {
            return null;
          }
          ...
          try {
            ...
            return {
              ...
              authorName,
              message,
              body: body ? body.trim() : "", // 添加 body 字段
