...
function getAllCommits(branchName, targetBranch, author, timeRange) {
  try {
    ...
    let baseCommand = `git log --pretty=format:"%H|%an|%s|%b|%ai"`;
    ...
    try {
      ...
      const commits = output
        ...
        .map((commit) => {
          ...
          // 前3个字段是固定的：hash, author, message
          const hash = parts[0];
          const authorName = parts[1];
          const message = parts[2];

          // 如果只有4个部分，说明没有 body，最后一个是 date
          // 如果有5个或更多部分，最后一个是 date，中间的都是 body
          let body = "";
          let date = "";
          ...
          if (parts.length === 4) {
            ...
            body = parts.slice(3, parts.length - 1).join("|");
          }
