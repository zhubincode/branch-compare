  ...
  const { author } = await inquirer.prompt([
    {
      ...
      source: async (answersSoFar, input = '') => {
        ...
        return authorChoices.filter(choice => choice.toLowerCase().includes((input || '').toLowerCase()));
      }
    }
  ]);
  return author;
...
async function getCommits(sourceBranch, targetBranch, timeRange, author) {
  console.log(`正在获取提交信息：源分支=${sourceBranch}, 目标分支=${targetBranch}, 时间范围=${timeRange}, 作者=${author}`);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && pathname === '/ignored-commits') {
    ...
    req.on('end', async () => {
      try {
        ...
        try {
          ...
          console.log('请求体解析成功:', `${JSON.stringify(data).substring(0, 200)}...`);
          ...
          console.log('解析备注数据成功, 类型:', Array.isArray(commitRemarks) ? 'Array' : typeof commitRemarks);
        } catch (parseError) {
        ...
        if (!Array.isArray(commitRemarks)) {
          ...
          if (typeof commitRemarks === 'object' && commitRemarks !== null) {
            ...
            console.log('已将对象格式的备注转换为数组格式, 条数:', commitRemarks.length);
          } else {
        ...
        commitRemarks = commitRemarks.filter(
          remark => remark && remark.hash && typeof remark.hash === 'string' && remark.hash.trim().length > 0
        );
    ...
    try {
      ...
      try {
        // 验证提交哈希是否存在和有效
        const objectType = execSync(`git cat-file -t ${hash}`, { encoding: 'utf-8' }).trim();
        ...
        console.log(`响应数据结构: ${JSON.stringify(Object.keys(responseData))}`);
        console.log(`数据对象结构: ${JSON.stringify(Object.keys(responseData.data))}`);
...
async function main() {
  try {
    // 添加测试备注，直接验证备注功能是否工作
    // console.log('正在进行备注功能测试...');
    // try {
    //   // 读取当前备注
    //   const currentRemarks = await loadRemarks();
    //   console.log('当前备注数量:', currentRemarks.length);

    //   // 添加一个测试备注
    //   const testRemark = {
    //     hash: `test-hash-${Date.now()}`,
    //     content: `这是一个测试备注 - ${new Date().toLocaleString()}`,
    //     timestamp: new Date().toISOString()
    //   };
    //   currentRemarks.push(testRemark);
    //   console.log('添加测试备注:', testRemark);

    //   // 保存备注
    //   const saveResult = await saveRemarks(currentRemarks);
    //   console.log('测试备注保存结果:', saveResult);

    //   // 重新读取验证
    //   const verifyRemarks = await loadRemarks();
    //   console.log(
    //     '验证备注数量:',
    //     verifyRemarks.length,
    //     '测试备注是否存在:',
    //     verifyRemarks.some(r => r.hash === testRemark.hash)
    //   );
    // } catch (testError) {
    //   console.error('备注功能测试失败:', testError);
    // }

    // 先启动服务器
    await new Promise(resolve => {
      server.listen(3000, () => {
        console.log(chalk.blue('服务器已启动在端口 3000'));
        resolve();
      });
    });
    ...
    // 获取提交信息
    const commits = await getCommits(sourceBranch, targetBranch, timeRange, author);
    ...
    const markdownPath = path.resolve(process.cwd(), 'branch-diff.md');
    ...
    // 生成并保存 HTML 时间轴
    const html = await generateTimelineHTML(commits, sourceBranch, targetBranch, author, timeRange, commitRemarks);
    const htmlPath = path.resolve(process.cwd(), 'branch-timeline.html');
